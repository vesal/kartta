...
function findRouteHERE(from, to, apikey, useSample = false) {
    ...
    const hereRouter = mapWrapper.L.Routing.OSRMv1.extend({
        options: {
        ...
        route: function(waypoints, callback, context, options) {
            if (useSample) {
                ...
                const routeObjs = processRouteData(sampleRouteData);
                setTimeout(() => {
                ...
                fetch(proxyUrl)
                    .then(res => res.text())
                    .then(text => {
                      const routeObjs =  processRouteData(text);
                      callback.call(context, null, routeObjs);
                    })
                    .catch(err => {
            ...
            function processRouteData(text) {
              ...
              for (const route of data.routes) {
                if (route.sections) {
                  for (const section of route.sections) {
                    ...
                    if (routeObj) {
                      routeObj.name = `Reitti ${routeIndex} = ${(routeObj.summary.totalDistance/1000).toFixed(1)} km`;
            ...
            function processSection(section, waypoints) {
              ...
              const instructions = actions.map(a => {
 ...
 function findRouteOSRM(from, to, callback) {
    ...
    if (!mapWrapper.L.Routing) {
      fetch('https://router.project-osrm.org/route/v1/driving/24.9384,60.1699;24.6559,60.2055?overview=full&geometries=geojson')
        .then(res => res.json())
        .then(data => {
          const coords = data.routes[0].geometry.coordinates.map(c => [c[1], c[0]]);
          mapWrapper.L.polyline(coords, { color: 'blue' }).addTo(this.map);
        });
    }
